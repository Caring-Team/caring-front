...
export default function RecommendStart() {
  ...
  const [recommendationResult, setRecommendationResult] = useState(null); // ⭐ API 결과 저장용
    ...
    try {
      ...
      if (!profile) {
        ...
        try {
          ...
          const newRes = await getMyElderlyProfiles();
      ...
      if (responseData.data) {
        if (responseData.data.data?.institutions) {
      ...
      await AsyncStorage.setItem("recommend_result", JSON.stringify(institutions));
setRecommendationResult(institutions);
