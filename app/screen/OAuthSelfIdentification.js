...
import { sendOAuth2CertificationCode } from "../api/auth/auth.api";

export default function OAuthSelfIdentification() {
  ...
  const handleSubmit = async () => {
    ...
    console.log("📌 [OAuth SelfIdentification] 인증 요청:", payload);

    try {
      // 임시 토큰은 이미 저장되어 있으므로 interceptor가 자동으로 헤더에 추가
      const response = await sendOAuth2CertificationCode(payload);
      console.log("✅ OAuth 인증번호 요청 성공:", response.data);

      // 다음 페이지로 데이터 전달
      router.push({
        pathname: "/screen/OAuthIdentificationNumber",
        params: {
          provider,
          name: form.name,
          birth_date,
      ...
      console.error("❌ OAuth 인증번호 요청 실패:", error.response?.data);
      const errorMessage =
        error.response?.data?.message ||
        "인증번호 요청에 실패했습니다. 다시 시도해주세요.";
      Alert.alert("인증 요청 실패", errorMessage);
    }
  };
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            {provider} 계정 연동을 위해 본인인증을 진행해주세요
          </Text>
        </View>
